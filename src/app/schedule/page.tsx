...
import { Clock, MapPin, User, Calendar, Home, Users, UserCheck, Coffee, AlertCircle } from "lucide-react"
...
export default function SchedulePage() {
        ...
        <SidebarInset>
          <header className="flex h-16 shrink-0 items-center gap-2 border-b border-gray-200 px-4 bg-white">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-lg font-semibold text-black">Escala de Trabalho - Barbearia</h1>
            </div>
          </header>
  ...
  if (schedulesError) {
    toast.error("Erro ao carregar sua escala", {
      description: `Não foi possível carregar os dados da escala. Detalhes: ${schedulesError || "Erro desconhecido."}`,
      duration: 5000,
    })
        ...
        <SidebarInset>
          ...
          <div className="flex items-center justify-center min-h-screen">
            <div className="text-center">
              ...
              <p className="text-lg font-medium text-red-600 mb-2">Ops! Algo deu errado</p>
              <p className="text-gray-600 mb-4">Não conseguimos carregar sua escala de trabalho</p>
              <Button onClick={refetchSchedules} className="bg-black text-white px-6 py-2 rounded hover:bg-gray-800">
                  ...
                  <div className="space-y-1">
                    ...
                    {schedules && schedules.length > 0 && (
                      <p className="text-xs mt-2 opacity-75">
                        Última atualização: {schedules[0].barber?.name} em {schedules[0].location?.name}
                      </p>
                    )}
                  </div>
                </div>
                        ...
                        <CardContent>
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="font-bold text-lg text-black">{barbersInLocation.length}</span>
                            </div>
                            <div className="space-y-1">
                              <p className="text-xs font-medium text-gray-700 mb-2">Equipe desta unidade:</p>
                              {barbersInLocation.slice(0, 3).map((barber) => (
                                <div key={barber.id} className="text-sm text-gray-600 flex items-center gap-1">
                                  <UserCheck className="h-3 w-3" />
                                  {barber.name}
                                  {currentBarber === barber.id && (
                                    <Badge className="ml-1 bg-black text-white text-xs">Você</Badge>
                                  )}
                                </div>
                              ))}
                              {barbersInLocation.length > 3 && (
                                <div className="text-xs text-gray-500 italic">
                                  +{barbersInLocation.length - 3} outros barbeiros
                                </div>
                              )}
                            </div>
                          </div>
              ...
              <Card className="border border-gray-200 shadow-lg bg-white">
                <CardHeader>
                  ...
                  <CardDescription className="text-gray-600">
                    Veja onde cada barbeiro trabalha durante a semana - Sua escala está destacada
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {!schedules || schedules.length === 0 ? (
                    <div className="text-center py-12 text-gray-500">
                      ...
                      <p className="text-sm">Entre em contato com a gerência para verificar sua escala</p>
                    </div>
                  ) : (
                              ...
                              .map((day) => (
                                ...
                                >
                                  <div className="flex flex-col items-center">
                                    ...
                                    <span className="text-xs text-gray-600 font-normal">{day.label.split("-")[0]}</span>
                                  </div>
                                </th>
                              ))}
                          </tr>
                          ...
                          {barbers?.map((barber, index) => (
                            <tr
                              ...
                              className={`border-t border-gray-200 hover:bg-gray-50 ${
                                currentBarber === barber.id ? "bg-yellow-50 border-yellow-200" : ""
                              }`}
                            >
                              <td className="p-3 font-medium text-black">
                                <div className="flex items-center gap-2">
                                  <div
                                    className={`w-2 h-2 rounded-full ${currentBarber === barber.id ? "bg-yellow-500" : "bg-gray-300"}`}
                                  />
                                  {barber.name}
                                  {currentBarber === barber.id && (
                                    <Badge className="bg-black text-white text-xs">Sua Escala</Badge>
                                  )}
                                </div>
                              </td>
                              {weekDays
                                    ...
                                    <td key={day.value} className="p-3 text-center">
                                      {schedule ? (
                                        <div className="space-y-2">
                                          <Badge
                                            variant="outline"
                                            className={`${getLocationColor(schedule.location.name)} text-white text-xs border-0 font-medium`}
                                          >
                  ...
                  {locations?.map((location) => {
                    ...
                    return (
                      <Card key={location.id} className="border border-gray-200 shadow-lg bg-white">
                        <CardHeader>
                          <CardTitle className="flex items-center gap-2 text-black">
                            <div className={`w-4 h-4 rounded-full ${getLocationColor(location.name)}`} />
                            {location.name}
                          </CardTitle>
                          <CardDescription className="text-gray-600">
                            {barbersInLocation.length} barbeiro(s) escalado(s) para esta unidade
                          </CardDescription>
                        </CardHeader>
                        <CardContent>
                                ...
                                return (
                                  <div
                                    ...
                                    className={`p-4 border rounded-lg ${
                                      currentBarber === barber.id ? "border-yellow-300 bg-yellow-50" : "border-gray-200"
                                    }`}
                                  >
                    ...
                    return (
                      <Card
                        ...
                        className={`border shadow-lg bg-white ${
                          isCurrentUser ? "ring-2 ring-black border-black bg-yellow-50" : "border-gray-200"
                        }`}
                      >
                        <CardHeader>
                          <CardTitle className="flex items-center gap-2 text-black">
                            <div
                              className={`w-3 h-3 rounded-full ${isCurrentUser ? "bg-yellow-500" : "bg-gray-400"}`}
                            />
                            {barber.name}
                            {isCurrentUser && <Badge className="bg-black text-white">Sua Escala</Badge>}
                          </CardTitle>
                          <CardDescription className="text-gray-600">
                          ...
                          <div className="space-y-3">
                            {uniqueLocations.length === 0 ? (
                              <div className="text-center py-6 text-gray-500">
                                ...
                                <p className="text-sm">Sem horários definidos</p>
                                <p className="text-xs">Entre em contato com a gerência</p>
                              </div>
                            ) : (
                              uniqueLocations.map((locationName) => {
                                ...
                                return (
                                  <div key={locationName} className="p-3 border border-gray-200 rounded-lg bg-white">
                                    <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <div className="space-y-2">
                                      <p className="text-xs font-medium text-gray-700">Seus horários:</p>
