...
export default function PlansPage() {
  ...
  const mostPopularPlan = plans?.reduce(
    (prev, current) => ((current._count?.clients || 0) > (prev._count?.clients || 0) ? current : prev),
    plans?.[0],
  )
      ...
      <SidebarInset>
        <header className="flex h-16 shrink-0 items-center gap-2 border-b border-gray-200 px-4 bg-white">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-semibold text-black">Planos e Assinaturas - Clube Couto</h1>
          </div>
        </header>
        ...
        <div className="flex flex-1 flex-col gap-6 p-6 bg-gray-50">
          ...
          <div>
            ...
            <div className="grid gap-4 md:grid-cols-4">
              ...
              <Card className="border border-gray-200 shadow-lg bg-white">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium text-gray-600">Receita Mensal Recorrente</CardTitle>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-gray-500">Valor garantido mensalmente</p>
                </CardContent>
              </Card>

              <Card className="border border-gray-200 shadow-lg bg-white">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium text-gray-600">Plano Mais Procurado</CardTitle>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-gray-500">
                    {mostPopularPlan?._count?.clients || 0} cliente(s) assinante(s)
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="border border-gray-200 shadow-lg bg-white">
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-gray-500">Desconto no primeiro mês</p>
                </CardContent>
              </Card>
            </div>
          </div>
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    {plan.benefits && typeof plan.benefits === "object" && (
                                      <div className="space-y-1">
                                        <p className="text-xs font-medium text-gray-700 mb-2">Benefícios inclusos:</p>
              ...
              <Card className="border border-yellow-200 shadow-lg bg-yellow-50">
                <CardHeader>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-xl text-black">🎉 Promoção Especial Ativa</CardTitle>
                      <CardDescription className="text-gray-700">
                        50% de desconto no primeiro mês para novos assinantes - Oferta por tempo limitado!
                      </CardDescription>
                    </div>
                  </div>
                </CardHeader>
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    {plans
                      ...
                      .map((plan) => (
                        ...
                        >
                          <p className="text-2xl font-bold text-black">R$ {(Number(plan.price) / 2).toFixed(2)}</p>
                          <p className="text-sm font-medium text-gray-700">{plan.name}</p>
                          <p className="text-xs text-gray-500">Apenas no primeiro mês</p>
                          <Badge className="mt-2 bg-black text-white text-xs">ECONOMIA DE 50%</Badge>
                        </div>
                      ))}
                  </div>
                </CardContent>
                    ...
                    <div className="space-y-4">
                      {clientsLoading ? (
                        ...
                        <div className="text-center py-8 text-gray-500">
                          ...
                          <p className="text-lg font-medium mb-2">Nenhum assinante encontrado</p>
                          <p className="text-sm">Ainda não há clientes com planos ativos</p>
                        </div>
                      ) : (
                        recentSubscribers.map((client) => {
                          ...
                          const hasDiscount = client.createdAt ? hasRecentDiscount(client.createdAt) : false
                            ...
                            >
                              ...
                              <div className="flex items-center space-x-2">
                                <div className="text-right">
                                  <p className="font-semibold text-black">
                                    R$ {Number(client.plan?.price || 0).toFixed(2)}/mês
                                  </p>
                                  {client.planEndDate && (
                                    <p className="text-xs text-gray-500">
                                      Válido até: {new Date(client.planEndDate).toLocaleDateString("pt-BR")}
                                    </p>
                                  )}
                                </div>
                                {hasDiscount && <Badge className="bg-black text-white text-xs">50% OFF</Badge>}
                              </div>
                            </div>
                          )
                        })
            ...
            <TabsContent value="analytics" className="space-y-6">
              <div>
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  <Card className="border border-gray-200 shadow-lg bg-white">
                    <CardHeader>
                      <CardTitle className="text-black">Distribuição de Assinantes por Plano</CardTitle>
                      <CardDescription className="text-gray-600">
                      ...
                      <div className="space-y-4">
                        {plansLoading ? (
                          ...
                          plans
                            ...
                            .map((plan) => {
                              const percentage =
                                totalSubscribers > 0 ? ((plan._count?.clients || 0) / totalSubscribers) * 100 : 0
                              const getColorClass = (name: string) => {
                                if (name.includes("Premium")) return "bg-black"
                                if (name.includes("Cabelo")) return "bg-gray-800"
                                if (name.includes("Barba")) return "bg-gray-600"
                                ...
                                <div key={plan.id} className="space-y-2">
                                  <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-gray-600">
                                      {plan._count?.clients || 0} cliente(s) - {percentage.toFixed(1)}%
                                    </span>
                                  </div>
                  ...
                  <Card className="border border-gray-200 shadow-lg bg-white">
                    <CardHeader>
                      <CardTitle className="text-black">Receita Mensal por Plano</CardTitle>
                      <CardDescription className="text-gray-600">
                                ...
                                >
                                  <div className="flex items-center space-x-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-xs text-gray-500">{plan._count?.clients || 0} assinante(s)</p>
                                    </div>
                                  </div>
                                  <div className="text-right">
                                    <p className="font-semibold text-black">
                                    ...
                                    <p className="text-xs text-gray-500">{percentage.toFixed(1)}% da receita</p>
                                  </div>
                                </div>
                              )
                            })
